...
exports.mapToEvents = mapToEvents;
 ...
 */
function mapToEvent(obj) {
    const ts = shared_1.timeUtils.now();
    ...
    const multiplierValue = multiplierKeys.length > 0 ? (0, decoder_1.extractNumber)(obj[multiplierKeys[0]]) : null;
    ...
    const playerValue = playerKeys.length > 0 ? (0, decoder_1.extractNumber)(obj[playerKeys[0]]) : null;
    // Look for wager/amount/pool patterns
    const wagerKeys = keys.filter(k => /wager|amount|pool/i.test(k));
    const wagerValue = wagerKeys.length > 0 ? (0, decoder_1.extractNumber)(obj[wagerKeys[0]]) : null;
    // Look for countdown/timer patterns
    const timerKeys = keys.filter(k => /countdown|timer/i.test(k));
    const timerValue = timerKeys.length > 0 ? (0, decoder_1.extractNumber)(obj[timerKeys[0]]) : null;
    // Look for round start signals
    const startSignals = stringValues.some(v => /start|begin|new|init/i.test(v) ||
        /live|active|running/i.test(v));
    // Look for round end signals
    const endSignals = stringValues.some(v => /end|stop|finish|complete/i.test(v) ||
        /rug|crash|boom/i.test(v));
    // Look for heartbeat/ping signals
    const heartbeatSignals = stringValues.some(v => /ping|pong|heartbeat|keepalive/i.test(v));
