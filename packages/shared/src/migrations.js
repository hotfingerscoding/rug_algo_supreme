 ...
 */
async function runMigrations(db, logger) {
    const log = logger || new index_1.Logger();
  ...
  `);
    ...
    const versionResult = getVersionStmt.get('schema_version');
    ...
    // Get final version
    const finalVersionResult = getVersionStmt.get('schema_version');
