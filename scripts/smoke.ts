...
class SmokeTester {
  ...
  private dbPath = 'data/rugs.sqlite';
  ...
  private async testDatabase(): Promise<void> {
    try {
      ...
      this.addResult('Database File', 'FAIL', 'Error checking database', { error: errorMessage });
    }
  }
  ...
  private async testDatabaseTables(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.addResult('Database Tables', 'FAIL', 'Error checking database tables', { error: errorMessage });
    }
  }

  private async testLastRound(): Promise<void> {
    try {
      if (!fs.existsSync(this.dbPath)) {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.addResult('Last Round', 'FAIL', 'Error checking last round', { error: errorMessage });
    }
  }
  ...
  private async testThresholds(): Promise<void> {
    try {
  ...
  private async queryTable(db: any, query: string): Promise<any[]> {
    try {
  ...
  private async makeRequest(url: string): Promise<{ status: number; data: any }> {
    try {
      const { stdout } = await execAsync(`curl -s -w "%{http_code}" -o /tmp/response.json ${url}`);
      const status = parseInt(stdout.trim());
      const data = fs.existsSync('/tmp/response.json') ? 
        JSON.parse(fs.readFileSync('/tmp/response.json', 'utf8')) : {};
  ...
  private addResult(name: string, status: 'PASS' | 'WARN' | 'FAIL', message: string, details?: any): void {
